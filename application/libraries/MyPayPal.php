...
class MyPayPal {
    ...
    function SetExpressCheckout($products, $charges, $noshipping='0'){

        //Parameters for SetExpressCheckout, which will be sent to PayPal

        $padata  = 	'&METHOD=SetExpressCheckout';
        $padata .= 	'&RETURNURL='.urlencode($this->CI->config->item('PPL_RETURN_URL'));
        $padata .=	'&CANCELURL='.urlencode($this->CI->config->item('PPL_CANCEL_URL'));
        ...
        */
        ...
        $padata .=	'&PAYMENTREQUEST_0_CURRENCYCODE='.urlencode($this->CI->config->item('PPL_CURRENCY_CODE'));

        //paypal custom template

        $padata .=	'&LOCALECODE='.$this->CI->config->item('PPL_LANG'); //PayPal pages to match the language on your website;
        $padata .=	'&LOGOIMG='.$this->CI->config->item('PPL_LOGO_IMG'); //site logo
        ...
        if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
            ...
            $paypalmode = ($this->CI->config->item('PPL_MODE')=='sandbox') ? '.sandbox' : '';
    ...
    function DoExpressCheckoutPayment(){
        ...
        if(!empty($_SESSION['ppl_products'])&&!empty($_SESSION['ppl_charges'])){
            ...
            $padata  = 	'&TOKEN='.urlencode($this->CI->input->get('token'));
            $padata .= 	'&PAYERID='.urlencode($this->CI->input->get('PayerID'));
            $padata .= 	'&PAYMENTREQUEST_0_PAYMENTACTION='.urlencode("SALE");
    ...
    function GetTransactionDetails(){
        ...
        $httpParsedResponseAr = $this->PPHttpPost('GetExpressCheckoutDetails', $padata, $this->CI->config->item('PPL_API_USER'), $this->CI->config->item('PPL_API_PASSWORD'), $this->CI->config->item('PPL_API_SIGNATURE'), $this->CI->config->item('PPL_MODE'));
    ...
    function PPHttpPost($methodName_, $nvpStr_) {
        ...
        $API_UserName = urlencode($this->CI->config->item('PPL_API_USER'));
        $API_Password = urlencode($this->CI->config->item('PPL_API_PASSWORD'));
        $API_Signature = urlencode($this->CI->config->item('PPL_API_SIGNATURE'));
